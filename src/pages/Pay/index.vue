    ...
    <div class="pay-container">
      ...
      <div class="checkout-steps">
        ...
        <div class="submit">
          ...
          <a href="javascript:;" class="btn" @click.passive="open">立即支付</a>
        </div>
        <div class="otherpay">
          <div class="step-tit">
            <h5>其他支付方式</h5>
...
export default {
    ...
    async open() {
      ...
      try {
        ...
        this.$alert(`<img src='${imgUrl}' />`, "请使用微信扫码支付", {
          ...
          .then(() => {})
          .catch(() => {});
        //弹出消息同时循环向后台发送请求,获取该订单的支付状态数据
        //根据返回来的支付状态数据去决定要不要跳转到成功支付页面
        if (!this.timer) {
          this.timer = setInterval(async () => {
            ...
            if (result.code === 200) {
              //支付成功
              //停止定时器 跳转到支付成功页面,把当前的状态保存起来,以便用户点击我已成功支付时去判断
              this.status = 200;
              clearInterval(this.timer); //clearInterval 清楚定时器,停止给定编号的定时器,并没有清空存储编号的变量
              this.timer = null;
              this.$msgbox.close();
  ...
  computed: {
    ...
    totalAmount() {
      return this.$route.query.totalAmount;
    },
  },
};
</script>

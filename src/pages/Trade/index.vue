  ...
  <div class="trade-container">
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        ...
        ><br />
        <span>手机: {{ defaultAddress.phoneNum }}</span
        ><br />
      </div>
    </div>
    <div class="sub clearFix">
      <!-- <router-link class="subBtn" @click="submitOrder">提交订单</router-link> -->
...
export default {
  ...
  methods: {
    ...
    async submitOrder() {
      ...
      const orderId = await this.$store.dispatch("getSubmitOrder", {
        tradeNo,
        tradeInfo,
      });
      if (orderId) {
        this.$router.push(
          `/pay/?orderNo=${orderId}&totalAmount=${this.tradeInfo.totalAmount} `
        );
      }
    },
  },
  computed: {
    ...
    skuMount() {
      if (this.detailArrayList) {
        return this.detailArrayList.reduce((p, c) => {
          p += c.skuNum;
          return p;
        }, 0);
      }
